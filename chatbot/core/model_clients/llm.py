...
class LLMCore(CustomLLM):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Attributes:
            ...
            _model_id (str): The model ID of the LLM server.
            ...
            function_name (str): The function name for calling LLM server history.

        Example:
        ```python

        from meal_planning.core.utils.model_clients import LLMCore

        # For calling local LLM server
        llm = LLMCore(
        ...
        # For calling OpenAI-compatible 3rd party server
        llm = LLMCore(
        ...
        # For calling OpenAI server with websocket
        llm = LLMCore(
            model_id="gpt-4o",
            use_openai=True,
            OPENAI_API_KEY="your_openai_api_key",
        ...
        # For calling OpenAI server with chat history
        llm = LLMCore(
            model_id="gpt-4o",
            use_openai=True,
            OPENAI_API_KEY="your_openai_api_key",
            ...
            chat_store=CacheChatStore(
                # Configure your chat store here
            )
        )

        # Example usage
        response = llm.complete(prompt="Hello, how are you?").text
        ```
        """
    ...
    def setup_token_counter(self) -> None:
        ...
        if self.use_openai and "gpt" in self._model_id:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                self._model_id, cache_dir=self.cache_dir
            )

    def setup_websocket(self) -> None:
        """Initialize and start websocket connection"""
        if "gpt-4o" not in self._model_id:
            raise InitWebsocketOpenAIError(
                "Realtime API is only supported for GPT-4o family models."
            )
        ...
        try:
            ...
            raise InitWebsocketOpenAIError(
                f"Error initializing websocket connection: {e}"
            )
    ...
    def clean_call_history(self) -> None:
        ...
        if self.use_history and self.function_name:
            self.chat_store.clear_messages(
                f"{self.function_name}_{self.user_id}_{self.assistant_id}"
            )
    ...
    def format_chat_message(self, text: str) -> List[Dict]:
        ...
        try:
            ...
            chat_history = self.chat_store.get_chat_history(
                f"original_{self.user_id}_{self.assistant_id}", en_translate=False
            )
    ...
    def format_call_message(self, prompt: str, name: Optional[str] = None) -> str:
        ...
        if self.use_history and name:
            chat_history = self.chat_store.get_chat_history(
                f"{name}_{self.user_id}_{self.assistant_id}", en_translate=False
            )
    ...
    def call_server(self, messages: List[Dict], generate_type: str) -> str:
        ...
        url = self.base_url if not self.use_openai else self.openai_base_url
        ...
        if generate_type == "generate":
            try:
                ...
                response_text = response.model_dump()["choices"][0]["message"][
                    "content"
                ]
                return response_text.replace("{{", "{").replace("}}", "}")
            except requests.exceptions.RequestException as e:
                raise CallServerLLMError(
                    f"Error calling the LLM server at endpoint /generate: {e}"
                )
            ...
            try:
                ...
                raise CallServerLLMError(
                    f"Error calling the LLM server at endpoint /streaming: {e}"
                )
